...
app.get("/api/addRecipe", (req, res) => {
  ...
  const Insert_Recipe =
    "INSERT INTO recipe (idrecipe, recipeName, imgURL, recipeInstruction) VALUES ('" +
    idrecipe +
    "','" +
    recipeName +
    "','" +
...
app.get("/api/saved_recipes", (req, res) => {
  ...
  const Saved_Recipes =
    "Select recipe.recipeName as recipeName, recipe.imgURL as image From recipeApp.saved_recipe Join recipeApp.user On saved_recipe.id_user = user.iduser Join recipeApp.recipe On saved_recipe.recipe_saved = recipe.idrecipe Where iduser =" +
    iduser;
...
app.get("/api/created_recipes", (req, res) => {
  ...
  const Created_Recipes =
    "Select recipe.recipeName as recipeName, recipe.imgURL as image From recipeApp.created_recipe Join recipeApp.user On created_recipe.userid = user.iduser Join recipeApp.recipe On created_recipe.recipe_created = recipe.idrecipe Where iduser =" +
    iduser;
...
app.get("/getRecommendations", (req, res) => {
  ...
  if (req.query.id <= arr.length) {
    ...
    for (i = 0; i < result.length; i++) {
      ...
      connection.query(
        "SELECT recipeName FROM recipeApp.recipe where idrecipe =" +
          (result[i] + 1),
        function (err, names) {
...
function getCount() {
  return new Promise((resolve, reject) => {
    connection.query(
      "SELECT count(distinct(user_id)) as count FROM recipeApp.user_rating;",
      function (err, count) {
...
function getData(i) {
  return new Promise((resolve, reject) => {
    connection.query(
      "SELECT rating FROM recipeApp.user_rating where user_id =" + i,
      function (err, result) {
        var items = [];
...
app.get("/getUserid", (req, res) => {
  connection.query(
    "SELECT iduser FROM recipeApp.user where userEmail = " +
      "'" +
      req.query.email +
      "'",
